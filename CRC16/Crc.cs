...
namespace CRC
{
    public class Crc16Ccitt
    {
        ...
        public bool VerifyChecksum(byte[] bytesWithCrc)
        {
            if (bytesWithCrc == null || bytesWithCrc.Length < 2)
            {
                return false;
            }

            byte[] bytes = new byte[bytesWithCrc.Length - 2];
            Array.Copy(bytesWithCrc, bytes, bytes.Length);

            ushort crc = BitConverter.ToUInt16(bytesWithCrc, bytes.Length);
